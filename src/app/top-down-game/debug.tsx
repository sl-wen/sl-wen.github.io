...
export default function GameDebugPage() {
  ...
  const addTestResult = (result: string) => {
    console.log(`[GameDebug] ${result}`);
    setTestResults(prev => [...prev, `${new Date().toLocaleTimeString()}: ${result}`]);
  };

  useEffect(() => {
    addTestResult('开始游戏调试测试');
    ...
    const runTests = async () => {
      try {
        ...
        // 测试3: 检查游戏资源
        addTestResult('测试3: 检查游戏资源');
        const assets = [
          '/game/assets/images/dialog_borderbox.png',
          '/game/assets/images/menu_background.png',
          '/game/assets/images/game_over_background.png',
          '/game/assets/images/main_menu_background.png',
          '/game/assets/sprites/atlas/hero.png',
          '/game/assets/sprites/atlas/hero.json',
          '/game/assets/maps/main_map.json'
        ];
        ...
        for (const asset of assets) {
          try {
            ...
            if (response.ok) {
              addTestResult(`✓ ${asset} 存在`);
            } else {
              addTestResult(`✗ ${asset} 不存在 (${response.status})`);
            }
          } catch (error) {
            addTestResult(`✗ ${asset} 检查失败: ${error}`);
          }
        }

        // 测试4: 创建简单Phaser游戏
        addTestResult('测试4: 创建简单Phaser游戏');
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-4">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl font-bold mb-6">游戏调试页面</h1>
